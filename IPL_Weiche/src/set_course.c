...
int steps (void)
{
	...
	{
	GPIOC->BSRR |=0x0005000A;			HAL_Delay(ms); 					//wie eigenen sekunden zähler schreiben?
...
int set_course (_Bool weiche,int steps)
  {
	int countsteps =0;
	  ...
	  if (weiche = 1)//rechts
	  {
		  ...
		  {
		  GPIOC->BSRR |=0x0005000A;countspeps++;			HAL_Delay(ms); 					//wie eigenen sekunden zähler schreiben?
		  ...
		  //GPIOC->BSRR |=0x0000000F;		//0x00000008, Rücksetzen der Ausgänge?
	  ...
	  if (weiche = 0)//links
	  {
		  ...
		  set=0;
		  //GPIOC->BSRR |=0x000F0000;								// Rücksetzen
		  }
	  return set;
	}

...
#define H_BRIDGE_OFF	(GPIOC->BRR |=0x0001) //H_Bridge OFF PA3/(PC0 f�r test)
#define H_BRIDGE_ON 	(GPIOC->BSRR|=0x0001) //H_Bridge ON  PA3/(PC0 f�r test)
	H_BRIDGE_OFF;
}
...
void SET_COURSE (void)
  {
	...
	int i;
	int posstart =0;
	if (BUTTON_LEFT)			//wenn knopf gedr�ckt wurde PA1
	{
		posstart = POTI;
